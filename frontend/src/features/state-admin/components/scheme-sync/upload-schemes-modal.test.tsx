...
jest.mock('@/shared/components/common', () => ({
  UploadFileModal: ({ onSubmit }: { onSubmit: (f: File) => void }) => (
    <button type="button" onClick={() => onSubmit(new File(['x'], 'schemes.xls'))}>
      submit-upload
    </button>
  ),
  ToastContainer: () => null,
...
describe('UploadSchemesModal', () => {
  it('submits file with tenant code from auth store', async () => {
    const mutate = jest.fn()
    mockedHooks.useUploadSchemesMutation.mockReturnValue({
      mutate,
      isPending: false,
    } as never)
    mockedAuthStore.mockReturnValue('TN')
    const user = userEvent.setup()
    renderWithProviders(<UploadSchemesModal isOpen onClose={jest.fn()} />)
